...
(function () {
    ...
    async function fetchPages() {
        for (var i = 1; i <= maxPage; i++) {
            const response = await $.get(`${notPageUrl}-${i}.htm`);
    ...
    */
    ...
    function addCheckbox() {
        ...
        let allCateList = Array.from(new Set(pageContent.map(x => x.forumname)))
        checkList = allCateList;
        allCateList.forEach(cate => {
            checkboxArea.append($(`<div>
                                    <input class="kaze-checkbox" type="checkbox" kaze-data="${cate}" id="kaze-cate-${cate}" checked name="type"  />
                                    <label for="kaze-cate-${cate}">${cate}</label>
                                </div>`))
        })
